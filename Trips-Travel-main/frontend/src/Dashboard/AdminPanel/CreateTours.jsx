  ...
  const handleFileInputChange = async (event) => {
    ...
    if (file) {
      ...
      try {
        ...
        toast.error("Failed to upload image. Server not responding.");
      }
      setUploading(false);
    }
  };
      ...
      <div className="bg-white mx-6 p-6 md:p-8 rounded-lg text-center shadow-md w-full max-w-xl m-8 md:max-w-[80%] ">
        ...
        <div className="flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-2 md:space-y-3">
            ...
            <div>
              <label htmlFor="" className="block text-md md:text-lg font-medium text-GrayColor">Tour Photo (Drag and Drop anywhere)</label>
              <div className="flex items-center gap-4 mt-2">
                {formData.photo && (
                  <img src={formData.photo} alt="Preview" className="w-[80px] h-[80px] object-cover border-2 shadow-sm rounded-md" />
                )}
                <input
                  type="file"
                  accept="image/*"
              ...
              {uploading && <p className="text-gray-500 text-sm mt-1">Uploading securely to Cloudinary...</p>}
            </div>
