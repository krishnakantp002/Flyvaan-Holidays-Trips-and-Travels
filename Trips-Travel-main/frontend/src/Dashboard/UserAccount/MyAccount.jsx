...
const MyAccount = () => {
  ...
  React.useEffect(() => {
    ...
    if (paymentStatus === "success") {
      toast.success("Payment completed securely via PayU!");
      // Clean up URL
          ...
          <div className="col-span-2 md:px-[30px] ">
            <div className="flex">
              <button
                ...
                className={`${tab === "bookings" && "bg-GrayColor text-white font-bold"
              ...
              <button
                onClick={() => setTab("settings")}
                className={`${tab === "settings" && "bg-GrayColor text-white font-bold"
                  } p-2 mr-5 px-3 lg:px-5 rounded-md text-HeadingColor font-semibold text-[13px] md:text-[14px] lg:text-[16px] leading-7 border border-solid border-Color`}
              >
                Profile Settings
              </button>
