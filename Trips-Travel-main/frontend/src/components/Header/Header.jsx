  ...
  return (
    <header ref={headerRef} className="transition-all shadow-md duration-300">
      <nav className="container mx-auto px-5 flex justify-between items-center py-2">
        {role === "admin" ? (
          <div className="h-8 md:h-12 w-32 md:w-48 flex items-center md:hidden">
            <img src={Logo} alt="" className="h-full w-full object-contain scale-[2] origin-left pointer-events-none" />
          </div>
        ) : (
          <div className="h-8 md:h-12 w-32 md:w-48 flex items-center">
            ...
            <ul className="flex flex-col item-center h-full justify-center gap-10">
              ...
              {role === "admin" && (
                <>
                  ...
                  <Link to="/all-contacts" onClick={handleMenuToggle}>
                    Messages
                  </Link>
                  <Link to="/admin-requests" onClick={handleMenuToggle}>
                    Requests
                  </Link>
                  <Link to="/admin-gallery" onClick={handleMenuToggle}>
                    Gallery
                  </Link>
                </>
              )}
        ...
        {role === "admin" ? (
          <ul className="md:flex hidden space-x-8">
            ...
            <Link to="/all-contacts">Messages</Link>
